...
let numIndex:number = 3;
let numValue:number = 11;

function splice(array:number[], index:number, value:number) {
...
let shoppingCart:string[] = ["Shoes", "Bags", "Clothes", "Sports"];
...
let facNumber:number = 10;

function facNums(facNum:number) {
    var res = facNum;
...
function func() {
    const arr:number[] = [9, 38, 45, -25, 42, -12, 98, -98, -198, 298];
    let nonNegArray:number[] = []
    let x:number = 0;
 ...
 let calArray:number[] = [12, 1, 36, 4];

 function cal(calArray:number[]) {
    let total:number = 0;
    let i:number = 0;
 ...
 let listOfTemps:number[] = [0, 10, 20, 30, 40];
 let newListOfTemps:number[] = [];
