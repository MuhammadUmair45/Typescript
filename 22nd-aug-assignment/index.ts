...
let radiusValue:number = 2;
//let pi = Math.PI;
let pi:number = 3.14;
const areaOfCircle = (radiusValue:number, pi:number) => {
...
// Initializing numbers array
let grades:number[] = [50, 13, 12, 62, 71, 43, 63, 12, 94, 48];

// Declaring empty New array
let newGradesArray:number[] = [];
...
var array:number[] = [3 , 6, 2, 56, 32, 5, 89, 32, 90];
var largest= 0;

for (let i:number=0; i<=largest; i++){
    if (array[i] > largest) {
        var largest = array[i];
    }
}
